...
export class ValidationErrorMiddlewareProvider implements Provider<Middleware> {
  ...
  /**
   * Handle errors
   * If the request url is `/employees`, customize the error message.
   * @param context
   * @param err
   */
  handleError(
    ...
    if (context.request.url === '/employees') {
      // if this is a validation error from the POST method, customize it
